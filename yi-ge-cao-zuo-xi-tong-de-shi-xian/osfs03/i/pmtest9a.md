...
LABEL_BEGIN:
	...
	; 以 LABEL_GO_BACK_TO_REAL 开始的这片内存区域，存储着机器指令 
	; BYTE1			BYTE2					BYTE3			BYTE4				BYTE5
	; OEAH 						Offset 								Segment
	; mov	[LABEL_GO_BACK_TO_REAL+3], ax 把第3个字节开始的2个字节替换为ax，初始序号是0，第3个字节是上面的BYTE4。
...
LABEL_MEM_CHK_OK:
	...
	; IdtPtr 是 IDT 寄存器，它的结构是：前16位是界限，后32位是基地址。
	; 4个字节，每个字节是8位，共32位
...
; 初始化8259A
; 结合《操作系统真相还原》7.5节阅读代码。《一个操作系统的实现》根本就没有解释为何写入ICW1的数据需要是011h？
; Init8259A ---------------------------------------------------------------------------------------------
Init8259A:
	; 将 011h 转成 0000 0001 0001 b，才能理解是如何向ICW1写入数据的
	mov	al, 011h
