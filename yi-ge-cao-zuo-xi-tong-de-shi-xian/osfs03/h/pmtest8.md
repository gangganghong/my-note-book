...
SetupPaging:
	...
	; 可用内存需要多少个页表？一个页表表示的内存是1024*4KB=4M
	xor	edx, edx
	mov	eax, [dwMemSize]
...
1. 线性地址的高10位乘以4，再加上从cr3中获取的页目录的物理地址，等于页目录项的物理地址A。
2. 线性地址的中间10位乘以4，再加上A，等于页表的物理地址B。
3. 线性地址的低12位，加上B，等于物理地址C。

线性地址的高10位、中间10位是偏移量，分别是在页目录中、页表中的偏移量，单位是4字节。线性地址的低12位是在页中的偏移量，单位是1字节。

cr3存储的是页目录表的物理地址。

### 难点

进一步替换分页机制，耗费了很长时间，仍不能理解。
